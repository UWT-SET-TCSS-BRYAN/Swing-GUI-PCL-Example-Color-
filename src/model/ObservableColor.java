 ...
 * @version Autumn 2015
 */
public class ObservableColor implements PropertyChangeEnabledMutableColor {

    /**
     * Error message used when client code attempts to set the value for Red
     * outside the valid range.
     */
    private static final String ERROR_MESSAGE_RED = "The value for red must be [0-255].";

    /**
     * Error message used when client code attempts to set the value for Green
     * outside the valid range.
     */
    private static final String ERROR_MESSAGE_GREEN = "The value for green must be [0-255].";

    /**
     * Error message used when client code attempts to set the value for Blue
     * outside the valid range.
     */
    private static final String ERROR_MESSAGE_BLUE = "The value for blue must be [0-255].";
     ...
     */
    public ObservableColor(final int theRed, 
                           final int theGreen, 
                           final int theBlue) {
        super();
        if (theRed < MIN_VALUE || theRed > MAX_VALUE) {
            throw new IllegalArgumentException(ERROR_MESSAGE_RED);
        }
        if (theGreen < MIN_VALUE || theGreen > MAX_VALUE) {
            throw new IllegalArgumentException(ERROR_MESSAGE_GREEN);
        }
        if (theBlue < MIN_VALUE || theBlue > MAX_VALUE) {
            throw new IllegalArgumentException(ERROR_MESSAGE_BLUE);
        }
        myRed = theRed;
        myGreen = theGreen;
        myBlue = theBlue;
